...
class AiService {
  ...
  Future<AiChatResult> sendConversation({
    ...
    if (onStreamDelta != null) {
      try {
        return await _sendStreaming(payload, token, onStreamDelta);
      } catch (error) {
    ...
    return _sendJson(payload, token);
  }

  Future<AiChatResult> _sendJson(Map<String, dynamic> payload, String token) async {
    ...
    final result = AiChatResult.fromJson(decoded);
    ...
    try {
      ...
      try {
        final response = await _withRetry(() {
          final request = http.Request('POST', _endpoint('groqChatStream'))
            ..headers.addAll({
              'Authorization': 'Bearer $token',
              'Content-Type': 'application/json',
        ...
        if (!contentType.contains('text/event-stream')) {
          ...
          final result = AiChatResult.fromJson(decoded, streamed: true);
      ...
      final fallback = await _sendJson(payload, token);
