...
class _TechnicianPremiumDashboardState extends State<TechnicianPremiumDashboard>
  ...
  Widget build(BuildContext context) {
    debugPrint('[Dashboard] build triggered');
    final currentUid = _auth.currentUser?.uid;
    debugPrint('[Dashboard] currentUid = $currentUid, _uid = $_uid, _profileStream = ${_profileStream != null}');
    if (currentUid != null &&
        (_uid != currentUid || _profileStream == null)) {
      debugPrint('[Dashboard] calling _configureStreams');
      _configureStreams();
    }

    final uid = _uid ?? currentUid;
    if (uid == null || _profileStream == null) {
      debugPrint('[Dashboard] Returning SizedBox.shrink() because uid or _profileStream is null');
    ...
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, profileSnapshot) {
        debugPrint('[Dashboard] profileSnapshot hasData: ${profileSnapshot.hasData}, hasError: ${profileSnapshot.hasError}');
        if (profileSnapshot.hasError) {
           debugPrint('[Dashboard] profile stream error: ${profileSnapshot.error}');
        }
  ...
  Widget _buildLiveStatisticsRow({
